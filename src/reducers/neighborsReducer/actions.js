...
const setNodes = dispatch => {
  ...
  const indexedCourses = indexForGraph
    .slice(students.length + teachers.length, indexForGraph.length)
    .map(data => ({
      ...data,
      label: data.name,
      shape: "hexagon",
...
export const setEdges = (
  ...
  for (const parent of parents) {
    for (const child of children) {
      if (
        parent[parentProp].some(
          prop => prop.toString() === child[childrenProp].toString()
        )
      ) {
        connections.push({
          from: parent.id,
